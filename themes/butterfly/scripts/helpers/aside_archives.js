...
hexo.extend.helper.register('aside_archives', function (options = {}) {
  ...
  if (len > Judge) {
    result += `<a class="card-more-btn" href="${this.url_for(archiveDir)}/" target="_parent" title="${this._p('aside.more_button')}">
  ...
  for (let i = 0; i < Judge; i++) {
    ...
    result += `<a class="card-archive-list-link" target="_blank" href="${link(item)}">`
